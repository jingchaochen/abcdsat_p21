...
enum clause_Type{LONG=0, REMOVEDCS, MIDSZ, SMALL};
...
public:
    ...
    int     nFreeVars  ()      const;
    int     FixedVars  ()      const;
    int     nUnfixedVars()     const   { return (int)nVars()  - (trail_lim.size() == 0 ? trail.size() : trail_lim[0]); }
    ...
    bool      BCDmode;
    bool      inprocessmode;
    int       WVSIDSmode;  //Weight Variable State Independent Decaying Sum
    ...
    uint64_t nbRemovedClauses, nbBin,nbUn,nbReduceDB,starts, decisions, rnd_decisions;
    uint64_t propagations, conflicts;
    uint64_t dec_vars, clauses_literals, learnts_literals;
    long curRestart;
//---------------------------------
    int  *varRange;
    void fast_bcd();
...
//-------------------------------------
    ...
    void solveEqu(int *equRepr, int equVars, vec<lbool> & Amodel);
    void s_extend ();
...
public:
    ...
    bool                remove_satisfied; // Indicates whether linear scan for satisfied clauses is performed in 'simplify'.
    vec<unsigned int>   permDiff;          // the current conflict number is Used to count the number of  LBD
    ...
    Lit      pure_pickBranchLit    ();                                                // Return the next decision variable.
    void     newDecisionLevel ();                                                      // Begins a new decision level.
    void     uncheckedEnqueue (Lit p, CRef from = CRef_Undef);                         // Enqueue a literal. Assumes value of literal is undefined.
    bool     enqueue          (Lit p, CRef from = CRef_Undef);                         // Test if fact 'p' contradicts current state, enqueue otherwise.
    CRef     propagate        ();                                                      // Perform unit propagation. Returns possibly conflicting clause.
    void     cancelUntil      (int level);                                             // Backtrack until a certain level.
    void     pure_analyze(CRef confl, vec<Lit>& out_learnt, int& out_btlevel,unsigned int &lbd);
    void     analyzeFinal     (Lit p, vec<Lit>& out_conflict);                         // COULD THIS BE IMPLEMENTED BY THE ORDINARIY "analyze" BY SOME REASONABLE GENERALIZATION?
    bool     litRedundant     (Lit p, uint32_t abstract_levels);                       // (helper method for 'analyze()')
    lbool    pure_search           (int nof_conflicts);                                     // Search for a given number of conflicts.
    lbool    pure_solve       ();                            // Main solve method
    void     pure_reduceDB    ();                            // Reduce the set of learnt clauses.
...
//mixed mode: conflict history-based
     uint64_t     next_simplify_small;
     double       alpha,beta;
     double       alpha_dec;
     double       min_alpha;
     int          timer;
     unsigned int small_lbd_lim;
     int          init_trailsize;
     int          learnt_small;
    ...
    vec<uint32_t> decisionTime, varBumpCnt;
    ...
    vec<Lit>    new_learntcls;
     ...
     void         varBumpVSIDSactivity  (Var v, double mult);    // Increase a variable with the current 'bump' value.
...
inline void Solver:: varActivityRescale(Var v) {
      if ( activity[v] > 1e100 ) {
...
inline void Solver::varBumpVSIDSactivity(Var v, double delta) {
    //  activity[v] += var_inc * mult;
      activity[v] += delta;
      varActivityRescale(v);
}
