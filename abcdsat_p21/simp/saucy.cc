...
static bool timeLimitReached(struct saucy *s)
{
  time_t now;
  time(&now);
...
static int do_search(struct saucy *s)
{
	...
	while (s->lev && not timeLimitReached(s)) {
                if(++loop>1000){
                  //  std::clog <<"c symmtry detect end at l=" <<loop<< std::endl;
                    break;
                }             
...
void saucy_search(struct saucy *s, const struct saucy_graph *g,	int directed, saucy_consumer *consumer,
	void *arg, struct saucy_stats *stats)
{
	int i, j, max = 0;
